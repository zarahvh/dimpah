...
- There are some complicated values in 'Term expires'. We simply take the first year we meet in the column as the right year. Make sure that this column is of type int.
...
Let's look into the questions when their seats are up again for the senators. Create a new dataframe from senators that only contains the 'Senator', 'State', 'Party', 'Occupation(s)', 'Years in Office' and 'Term expires' columns. Call it senators_seatup.
...
Next, we would like to determine the next time an election is held. This is the minimum value in the 'Term expires' column. Assign that value to a variable next_election and pring it out. 
...
So far so good. Let's next group observations together to gain composite insights. Let's look at the senators per US state. Use senators_seatup_next and the columns 'State' and 'Term expires' to display the number of terms that are up in the next election.



At least in 2020, there were quite a few senators up for re-election. How does it look for you? 

Finally, we wanted to look into the election challenges per party. Select 'Party' and 'Term expires' and group by party to display the results, please.
