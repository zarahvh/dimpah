...
Finally, check how many democrats, republicans and independents there are in congress_114. As you can see this is only a subset of the 100 senators. Type in `congress_114.iloc[:,2].value_counts()` to select column 3 with iloc and apply value_counts().
...
As we already have a congress_114_result data frame, we just need to create a new column in it with the new data. Create a new column cluster_5 with `congress_114_result["cluster_5"] = np.array(kmeans_5.labels_)`
